...
import { BookOpen, Clapperboard, Headphones, Film } from "lucide-react";
  ...
  return (
    <section id="creative" className="sticky top-0 z-[80] flex h-screen flex-col justify-center overflow-hidden bg-paper py-8 scroll-mt-20 sm:py-10">
      <div className="container">
        ...
        <div className="grid gap-px overflow-hidden rounded-xl border-2 border-ink bg-ink sm:grid-cols-2 md:grid-cols-4">
          {creativeWork.map((c, i) => {
            ...
            return (
              <Reveal key={c.title} delay={i * 0.1} className="h-full">
                <div className="group flex h-full flex-col justify-between bg-paper p-4 transition-colors hover:bg-signal hover:text-paper">
                  ...
                  <div className="mt-6">
                    <h3 className="font-serif text-base italic leading-tight sm:text-lg">{c.title}</h3>
                    <p className="mt-2 line-clamp-3 font-serif text-xs leading-snug text-muted-foreground group-hover:text-paper/90 sm:text-sm">
                      {c.body}
                    </p>
                  </div>
                </div>
